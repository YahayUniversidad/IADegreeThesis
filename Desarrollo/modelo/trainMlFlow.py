...
def entrenar_modelo(modelo_cnn, X_train, y_train_list, X_test, y_test_list, scaler, features_numericas):
## def entrenar_modelo(modelo_cnn, X_train, y_train_list, X_test, y_test_list):
    """Entrena el modelo CNN multi-horizonte a 18 meses con MLflow tracking"""
    
    mlflow.set_tracking_uri("http://localhost:5000")  # o la URL de tu servidor MLflow
    # mlflow.set_experiment("cnn-multi-horizonte-001")
    ...
    # with mlflow.start_run(nested=True) as run:
        ...
        mlflow.log_param("ventana_cnn", 6)
        mlflow.log_param("max_horizonte", 18)
        mlflow.log_param("model_type", "CNN_MultiOutput")
        mlflow.log_param("optimizer", "adam")
        mlflow.log_param("loss", "binary_crossentropy")
        mlflow.log_param("activation_output", "sigmoid")
        
        # Log de arquitectura resumida
        modelo_cnn.summary(print_fn=lambda x: mlflow.log_text(x, "model_summary.txt"))
        
        callbacks = [
            EarlyStopping(monitor="val_loss", patience=10, restore_best_weights=True),
            ModelCheckpoint(
                "modelos_cnn/best_model_multi_18m.keras",
                monitor="val_loss",
                save_best_only=True,
            ),
        ]
        ...
        # Log de métricas de entrenamiento (última época)
        for metric in historia.history.keys():
            mlflow.log_metric(metric, historia.history[metric][-1])
        
        # Log de pérdida por época (gráfica)
        for epoch in range(len(historia.history['loss'])):
            mlflow.log_metric("train_loss_epoch", historia.history['loss'][epoch], step=epoch)
            mlflow.log_metric("val_loss_epoch", historia.history['val_loss'][epoch], step=epoch)
        ...
        mlflow.keras.log_model(
            modelo_cnn,
            artifact_path="model",
            registered_model_name="CNN_MultiHorizonte_18m",
            # se retira para subir a mlflow
        ...
        config = {
            "ventana_cnn": 6,
            "max_horizonte": 18,
            "features_count": len(features_numericas),
        }
...
def evaluar_y_guardar(
    ...
    modelo_cnn.save('modelos_cnn/modelo_cnn_multi_18m.keras')
...
def main():
    ...
    # 6. Configurar MLflow (opcional para tracking remoto)
    mlflow.set_tracking_uri("http://localhost:5000")
    
    # Iniciar experimento
    mlflow.set_experiment("cnn-multi-horizonte-002")
        
    # 7. Construir el modelo
    modelo_cnn = construir_modelo(X_train, MAX_HORIZONTE)
    
    # 8. Entrenar el modelo (pasa X_test, y_test_list como args adicionales)
    # historia = entrenar_modelo(modelo_cnn, X_train, y_train_list, X_test, y_test_list)
    historia = entrenar_modelo(modelo_cnn, X_train, y_train_list, X_test, y_test_list, scaler, features_numericas)
    
    # 9. Evaluar y guardar artefactos
    evaluar_y_guardar(
        modelo_cnn,
        X_test,
        y_test_list,
