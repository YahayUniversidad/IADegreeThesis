...
La Figura 5.1 muestra correlaciones elevadas entre algunas variables. En particular, el documento original reporta una relación cercana a 1 entre \texttt{num\_creditos} y \texttt{creditos\_cerrados}, así como entre \texttt{monto\_total} y \texttt{monto\_promedio}. Estas relaciones pueden generar redundancia, aunque la multicolinealidad no afecta de la misma manera a árboles y redes neuronales. Los valores de VIF (del inglés \textit{Variance Inflation Factor}) mencionados en la versión original no se conservan como evidencia principal porque no se documenta su cálculo completo.
...
\begin{figure}[H]
    ...
    \caption{AUC-ROC calculada individualmente para cada variable predictora, evaluada contra la etiqueta \texttt{crisis\_flag}. El AUC-ROC mide la capacidad de cada variable para distinguir entre bloques con y sin crisis, sin considerar interacciones con otras variables.}
    \label{fig:auc_roc}
\end{figure}

Estos resultados requieren una lectura prudente. Las variables judiciales son atributos históricos disponibles antes del instante de predicción, por lo que su alta capacidad discriminativa puede reflejar una señal predictiva legítima. Sin embargo, dado que estas mismas variables intervienen en la definición de \texttt{crisis\_flag} (condiciones 1, 2 y 3), parte del poder predictivo observado es atribuible a la circularidad de constructo documentada en la Sección 4.5. La separación entre señal genuina y dependencia estructural requiere una ablación que excluya las variables judiciales del conjunto de predictores.
...
LightGBM produjo resultados para los $N = 18$ horizontes definidos. La Tabla 5.3 reproduce los horizontes documentados y el promedio reportado. El AUC-ROC disminuye de 0,874 en el horizonte de 1 mes a 0,702 en el de 18 meses. Sin embargo, la métrica más crítica es el recall, que se reduce drásticamente de 0,542 en el horizonte de 1 mes a 0,034 en el de 18 meses. Como se observa en la Tabla A.1, a partir del horizonte $h = 8$, el recall cae a 0, lo que indica que el modelo, con el umbral de clasificación de 0,5, no logra identificar ningún caso de crisis en horizontes intermedios y largos. Por lo tanto, aunque el sistema genera predicciones para los 18 horizontes, su utilidad operativa se concentra en los horizontes cortos $(h \le 6)$.
...
Bajo el criterio implementado de AUC-ROC promedio y en el escenario específico de este estudio (con los datos, la división temporal y las configuraciones documentadas), LightGBM fue seleccionado para el flujo de inferencia. La selección es coherente con los resultados disponibles, pero debe considerarse provisional por cuatro razones: no se documentan repeticiones; no existe una prueba estadística; el ajuste de hiperparámetros no fue equivalente; y las salidas de LightGBM y de las redes se estructuran de manera diferente.
...
El deterioro con el horizonte es visible en AUC-ROC y, con mayor intensidad, en recall. La cifra de recall igual a 0,034 a dieciocho meses significa que la clasificación binaria bajo el umbral 0,5 recupera una fracción muy pequeña de las crisis. Aunque el AUC-ROC se mantiene por encima de 0,70, no es suficiente para calificar estas predicciones como confiables o útiles sin análisis de umbral, calibración y costos de error. En el marco de este estudio, los resultados sugieren que la utilidad practicca del sistema se limita a los horizontes cortos $(h \le 6) $\\

El bajo rendimiento de CNN y MLP puede asociarse, de manera plausible, con la muestra secuencial reducida (solo bloques con $\ge$ 24 meses de historial generan secuencias), el desbalance de clases sin compensación en MLP, la longitud de las ventanas de seis meses y la configuración de entrenamiento documentada. No obstante, no se realizaron experimentos de ablación que modifiquen sistemáticamente estos factores para aislar su efecto. En consecuencia, estas explicaciones deben considerarse plausibles pero no como causas demostradas del comportamiento observado.\\
