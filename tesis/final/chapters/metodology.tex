...
La arquitectura integra cinco componentes: PostgreSQL 15 como repositorio operativo y analítico; Apache Airflow 2.10 como orquestador; Python 3.12 y Notebook Jupyter para el desarrollo de modelos; MLflow para el registro de experimentos; y Apache Superset para la visualización. La Figura~\ref{fig:arquitectura} resume las interacciones entre estos componentes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/MM-04 Arquitectura del Sistema Completo.png}
    \caption{Arquitectura general del prototipo de inteligencia de negocio y MLOps. Las fuentes se integran mediante ETL, los modelos se entrenan y registran en MLflow, y los resultados se visualizan en Apache Superset.}
...
\begin{table}[H]
    ...
    \hline
        \textbf{Fuente} & \textbf{Magnitud Aprox.} & \textbf{Contenido} \\
    \hline
        CABECERA\_PRESTAMOS & 1.044.194 filas & Información maestra de créditos, periodo 2015--2026 \\
    \hline
        AMORTIZACAL\_PRESTAMOS & 24.350.210 filas & Tabla de amortización mensual \\
    \hline
        RECUPERACION\_PRESTAMOS & 15.741 filas & Información de juicios y recuperaciones \\
    \hline
    \end{tabular}
...
La Tabla~\ref{tab:trazabilidad} documenta la trazabilidad de los datos desde las fuentes hasta las muestras de entrenamiento, verificada mediante consultas directas a PostgreSQL y MLflow.
...
\begin{table}[H]
    ...
    \hline
        Filas CABECERA\_PRESTAMOS & 1.044.194 & Registros maestros de créditos \\
    \hline
        Filas AMORTIZACAL\_PRESTAMOS & 24.350.210 & Cuotas de amortización \\
    \hline
        Filas RECUPERACION\_PRESTAMOS & 15.741 & Juicios y recuperaciones \\
    \hline
    ...
    \hline
        Secuencias generadas & 25.434 & Ventanas de 6 meses × features válidas \\
    \hline
    ...
    \hline
    \end{tabular}
    \caption{Trazabilidad de datos verificada mediante consultas directas a PostgreSQL y MLflow. Conteos extraídos de la ejecución del 29 de julio de 2026.}
    \label{tab:trazabilidad}
\end{table}

\textbf{Nota sobre la muestra efectiva}: La restricción de que cada bloque-mes debe tener al menos 24 meses de historial continuo (6 meses de ventana + 18 horizontes) reduce el número de secuencias disponibles. La vista materializada contiene 45.077 bloques-mes, de los cuales se generan 25.434 secuencias válidas. De éstas, 12.463 se destinan a entrenamiento, 5.340 a validación y 7.631 a prueba. La distribución de la variable objetivo en la MV es 39.660 bloques sin crisis (88\%) y 5.417 con crisis (12\%).

\section{Procesamiento y preparación de datos}

El procesamiento se implementa en una vista materializada y en tareas del DAG (por sus siglas en inglés, Gráfico Acíclico Dirigido). Las etapas documentadas son: integración de las tres fuentes, validación de claves, prevención de duplicados mediante la combinación de inserciones y actualizaciones de registros, agregación mensual y cálculo de características. El proyecto no aporta un registro completo de filas eliminadas, valores faltantes, criterios de tratamiento de atípicos ni conteos posteriores a cada filtro. Estas decisiones deben versionarse junto con el código para garantizar reproducibilidad.\\

Las redes neuronales requieren una representación secuencial. Para cada bloque se utiliza una ventana de seis meses con veintiun características por mes, lo que corresponde a una matriz de $6 \times 21$ y a 126 valores antes de cualquier transformación adicional. La referencia original a 128 valores se corrige porque no se documentan dos variables adicionales.

\section{Ingeniería de características y variable objetivo}

El proyecto calcula veintiuna características numéricas derivadas de las tres fuentes. La Tabla~\ref{tab:caracteristicas} presenta el diccionario completo extraído del código fuente.
...
La variable binaria \texttt{crisis\_flag} se construye mediante ocho condiciones heurísticas con pesos acumulativos, diseñadas en colaboración con el equipo de riesgos de la institución financiera objeto de estudio. El objetivo de esta regla es identificar bloques (combinación de mes, nivel de riesgo, sector económico y sucursal) que presentan un deterioro significativo en su comportamiento crediticio. Si la suma de pesos supera el umbral de 5, el bloque se etiqueta como crisis (1); de lo contrario, como no crisis (0).\\

Las ocho condiciones que componen la regla son:\\
\begin{itemize}
...
\textbf{Justificación de los umbrales y pesos}: La selección de estos valores se basó en la experiencia operativa del equipo de riesgos, quienes identificaron estas variables como los principales indicadores de deterioro crediticio en el contexto de la institución. Los umbrales específicos (por ejemplo, tasa\_judicial > 5\%, plazo\_promedio > 36 meses) reflejan puntos de quiebre históricamente observados en la cartera, donde el comportamiento de los créditos tiende a volverse crítico. Los pesos asignados (3, 2 y 1) representan la importancia relativa que los analistas asignan a cada condición: las variables judiciales (pesos 3 y 2) se consideran las señales más tempranas y contundentes de crisis, seguidas por la tasa de cierre (peso 2), mientras que las condiciones restantes (peso 1) actúan como indicadores complementarios.\\

\textbf{Validación experta}: La regla completa y su umbral de decisión ($\sum \text{pesos} \geq 5$) fueron validadas por el equipo de riesgos de la institución mediante un proceso de revisión de casos históricos. Los analistas evaluaron manualmente una muestra representativa de bloques etiquetados como crisis y no crisis, confirmando que la regla capturaba adecuadamente los escenarios de deterioro identificados en la práctica operativa. Esta validación, aunque cualitativa, asegura que la definición de crisis\_flag está alineada con el conocimiento tácito del negocio y no responde únicamente a criterios estadísticos arbitrarios.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/MM-07 Definicion de crisis_flag.png}
...
\textbf{Circularidad de constructo en la etiqueta}: Las condiciones 1, 2 y 3 de \texttt{crisis\_flag} utilizan variables judiciales (\texttt{creditos\_judiciales}, \texttt{tasa\_judicial}, \texttt{total\_costo\_judicial}) que también aparecen como características de entrada. Dado que el campo \texttt{judicial} es un atributo histórico del crédito (registrado al momento de la operación, antes del instante de predicción), no existe fuga de información temporal en sentido estricto. Sin embargo, el uso de las mismas variables tanto para definir la etiqueta como para predecirla genera una \textbf{circularidad de constructo}: la elevada importancia predictiva de estas variables (AUC-ROC individual $> 0,9$) refleja parcialmente la dependencia estructural con la definición de la etiqueta, y no necesariamente una capacidad genuina de anticipar crisis.\\

Para cuantificar la contribución real de la señal judicial más allá de esta circularidad, se requiere una ablación que excluya \texttt{tasa\_judicial}, \texttt{creditos\_judiciales} y \texttt{total\_costo\_judicial} del conjunto de predictores y compare el rendimiento del modelo con y sin estas variables.

\subsection{Generación de horizontes}

La salida considera dieciocho horizontes mensuales ($N = 18$), abarcando desde 1 hasta 18 meses en el futuro. Para una ventana de entrada de seis meses que termina en el mes $t$, la etiqueta del horizonte $h$ corresponde al estado futuro en el mes $t + h $, donde $h \in \{1, \dots, 18\}$. De esta forma, la ventana proporciona la información histórica (meses $t-5, \dots, t$) y los horizontes representan los meses futuros a predecir ($t+1,…,t+18$). Si bien el sistema genera predicciones para los 18 horizontes, el análisis de resultados (Capítulo 5) muestra que la fiabilidad del modelo es significativamente mayor en los horizontes cortos $(h \le 6) $.\\

CNN y MLP generan un vector de dieciocho probabilidades, mientras que LightGBM entrena un clasificador independiente por horizonte.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/MM-01 Ventana Temporal de Entrada (6 meses × 21 features).png}
...
A diferencia de las redes neuronales, LightGBM entrena 18 clasificadores independientes, uno por horizonte. Sin embargo, la entrada de cada clasificador es idéntica a la de CNN y MLP: un vector de $6 \times 21 = 126$ valores obtenido mediante aplanamiento de la ventana temporal, sin cálculo de estadísticas agregadas.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/MM-03 Comparacion de Arquitecturas de Modelos.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{images/MM-05 Flujo de Entrenamiento.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{images/MM-06 Flujo de Inferencia Diaria.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{images/MM-09 Diagrama de Integracion.png}
...
La Tabla~\ref{tab:recursos} documenta los recursos utilizados para el entrenamiento de los modelos, extraída del código y la documentación del proyecto.
...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    \caption{Recursos computacionales y versiones de software. Hardware de entrenamiento utilizado durante el desarrollo}
    \label{tab:recursos}
\end{table}
...
El proyecto utiliza los siguientes mecanismos para favorecer la reproducibilidad:
