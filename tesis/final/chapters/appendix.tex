...
\begin{table}[h!]
    ...
    \hline
    \end{tabular}
    \caption{Métricas completas de LightGBM para los $N=18$ horizontes temporales. La precisión y el recall se calculan con umbral de clasificación 0,5. Solo 5 de los 18 horizontes presentan recall superior a cero, lo que evidencia la dificultad del modelo para identificar la clase minoritaria en horizontes intermedios y largos. Nota: Los horizontes con recall = 0 (a partir de $ h = 4 $ en algunos casos y consistentemente desde $ h = 8 $) indican que el modelo no es operativamente útil para esos plazos bajo la configuración evaluada. La utilidad práctica del sistema se limita a los horizontes cortos ($ h \le 6 $).}
    \label{tab:lgbm_18h}
\end{table}
